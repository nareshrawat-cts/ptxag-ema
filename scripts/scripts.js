...
/**
 * Applies section metadata (e.g. `Style`) to each section as classes.
 * This project's aem.js decorateSections() does not process section-metadata
 * blocks, so we handle it here: read the config table, add the style classes
 * to the parent section, then remove the metadata block from the DOM.
